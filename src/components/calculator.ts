...
const TBSA_ADULT_FRONT: Record<string, string> = {
...
const TBSA_ADULT_BACK: Record<string, string> = {
...
const TBSA_PEDS_FRONT: Record<string, string> = {
...
const TBSA_PEDS_BACK: Record<string, string> = {
...
const TBSA_ADULT_PCT: Record<string, number> = {
...
// Lund-Browder age table
interface LBAge { label: string; headTotal: number; eachThighTotal: number; eachLowerLegTotal: number; }
const LB_AGES: LBAge[] = [
...
function buildPedsPct(ageIdx: number): Record<string, number> {
...
function buildTbsaPainter(
  container: HTMLElement,
  frontPaths: Record<string, string>,
  ...
  function isInBody(cx: number, cy: number): boolean {
  ...
  function onPointerDown(e: PointerEvent): void {
    ...
    if (!pt || !isInBody(pt.x, pt.y)) return; // allow scroll outside body
    e.preventDefault();
    isPainting = true;
    saveSnap();
    paintAt(pt.x, pt.y);
  ...
  function onPointerMove(e: PointerEvent): void {
    ...
    const pt = clientToCanvas(e);
    if (pt) {
      if (lastPt) paintLine(lastPt.x, lastPt.y, pt.x, pt.y);
  ...
  clearBtn.addEventListener('click', () => {
    if (!confirm('Clear all burns?')) return;
    saveSnap();
    paintCanvas.getContext('2d')!.clearRect(0, 0, CW, CH);
    calcTbsa();
  });
...
const TBSA_PEDS_CALCULATOR: CalculatorDefinition = {
  ...
  customRender: (container: HTMLElement, onUpdate: (values: Record<string, number>) => void) => {
    let currentAgeIdx = 0;
    let painter: { destroy: () => void } | null = null;
...
const ICH_SCORE_CALCULATOR: CalculatorDefinition = {
  id: 'ich-score',
  title: 'ICH Score',
...
const FUNC_SCORE_CALCULATOR: CalculatorDefinition = {
  id: 'func-score',
  title: 'FUNC Score',
...
// -------------------------------------------------------------------
// RASS Calculator Definition (Richmond Agitation-Sedation Scale)
// -------------------------------------------------------------------
...
const ANION_GAP_CALCULATOR: CalculatorDefinition = {
  id: 'anion-gap',
...
const DELTA_GAP_CALCULATOR: CalculatorDefinition = {
  id: 'delta-gap',
...
const WINTERS_FORMULA_CALCULATOR: CalculatorDefinition = {
  id: 'winters-formula',
...
const OSMOLAR_GAP_CALCULATOR: CalculatorDefinition = {
  id: 'osmolar-gap',
  title: 'Osmolar Gap',
...
const STEWART_SIG_CALCULATOR: CalculatorDefinition = {
  id: 'stewart-sig',
