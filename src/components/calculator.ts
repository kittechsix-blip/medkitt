...
function buildBodyDiagram(
  ...
  clearBtn.addEventListener('click', () => {
    for (const r of allRegions) state[r.id] = false;
...
const TBSA_ADULT_CALCULATOR: CalculatorDefinition = {
  ...
  customRender: (container: HTMLElement, onUpdate: (values: Record<string, number>) => void) => {
    buildBodyDiagram(
      ...
      ADULT_PERINEUM,
      onUpdate,
      'Tap burned regions. Only mark 2nd degree (partial) and 3rd degree (full thickness) burns. Superficial (1st degree) burns are NOT included in TBSA.',
    );
  },
};
