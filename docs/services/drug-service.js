...
async function refreshFromSupabase() {
    const result = await supabaseFetch('drugs', 'select=*&order=sort_order');
...
async function mergeHardcodedDrugs() {
    const mod = await import('../data/drug-store.js');
    const hardcoded = mod.getAllDrugs();
    let added = false;
 ...
 */
export async function initDrugs() {
    if (initialized)
        return;
