...
// -------------------------------------------------------------------
export function renderConsultWizard(container, consultData) {
...
function renderStepScreen(container) {
  ...
  `;
    ...
    if (path.length > 0) {
        ...
        summary.innerHTML = path.map(entry => `<span class="cw-chip" style="background: ${accentColor}">${entry.choiceLabel}</span>`).join('');
  ...
  `;
    ...
    `;
        btn.addEventListener('click', (e) => {
            ...
            if (e.target.closest('.cw-option-info'))
                return;
